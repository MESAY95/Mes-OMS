...
router.get('/', async (req, res) => {
  try {
    ...
    // Transform materials to include name field for frontend compatibility
    const transformedMaterials = materials.map(material => ({
      ...material.toObject(),
      name: material.Material // Map Material field to name for frontend
    }));
...
router.get('/:id', async (req, res) => {
  try {
    ...
    // Transform material to include name field
...
router.post('/', async (req, res) => {
  try {
    ...
    const requiredFields = [
      'Material', 'MaterialCode', 'PackSize', 'Unit', 'UnitPrice', 
    ...
    const numericFields = [
      ...
      { field: 'ReorderQuantity', type: 'int' },
    ...
    const existingMaterial = await Material.findOne({ 
      $or: [
        ...
        { MaterialCode: { $regex: new RegExp(`^${req.body.MaterialCode}$`, 'i') } }
      ]
    });
    
    if (existingMaterial) {
      if (existingMaterial.Material.toLowerCase() === req.body.Material.toLowerCase()) {
        validationErrors.push(`Material with name "${req.body.Material}" already exists`);
      }
      if (existingMaterial.MaterialCode.toLowerCase() === req.body.MaterialCode.toLowerCase()) {
        validationErrors.push(`Material with code "${req.body.MaterialCode}" already exists`);
      }
    }
    ...
    const material = new Material({
      ...
      MaterialCode: req.body.MaterialCode.trim(),
    ...
    if (req.body.Material !== undefined) {
      if (!req.body.Material.trim()) {
        ...
        if (duplicateMaterial) {
          if (duplicateMaterial.Material.toLowerCase() === req.body.Material.toLowerCase()) {
            validationErrors.push(`Material with name "${req.body.Material}" already exists`);
          }
          if (req.body.MaterialCode && duplicateMaterial.MaterialCode.toLowerCase() === req.body.MaterialCode.toLowerCase()) {
            validationErrors.push(`Material with code "${req.body.MaterialCode}" already exists`);
          }
        } else {
